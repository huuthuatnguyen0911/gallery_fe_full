        ...
        <div class="grid md:grid-cols-2 grid-cols-1">
          <div class="blog p-6 rounded-lg bg-white max-w-sm mt-4">
            <form>
              ...
              <button
                class="w-full px-6 py-2.5 bg-green-600 text-white font-medium text-xs rounded shadow-md"
                :class="uploadStatus && 'bg-gray-600'"
                :disabled="uploadStatus"
                type="submit"
                @click.prevent="submitUpload"
              >
                {{ uploadStatus ? "Đang tải ảnh lên" : "Tải ảnh lên" }}
              </button>
            </form>
          </div>
          <!--Image Preview-->
...
// format image size to human-readalble unit
...
const getImagePreviews = (image) => {
  if (/\.(jpe?g|png)$/i.test(image.name) && image.size < 3000000) {
    ...
    error.value = "File ảnh không hỗ trợ hoặc kích thước lớn hơn 3MB";
